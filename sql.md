...
- Examples: `Amazon RDS` (`Relational`), `Google BigQuery` (`Data Warehousing`), `Amazon DynamoDB` (`NoSQL`)
...
- Examples: `Apache Cassandra` (`NoSQL`), Amazon Redis (`Data Warehousing`)
...
Apache Hive is an open-source data warehouse system that allows users to read, write, and manage large datasets using SQL-like queries. It's designed to operate at a large scale and is fault-tolerant, allowing for analytics on a massive scale. Hive is built on top of Apache Hadoop and is used by many companies, including social media outlets and corporations like Netflix and Amazon.
...
Teradata is one of the popular Relational Database Management System (RDBMS). It is mainly suitable for building large scale data warehousing applications. Teradata achieves this by the concept of parallelism. It is developed by the company called Teradata.

```sql
SELECT top 5 *
FROM tblEmployee
WHERE Employee_last_name = 'Singh';
```



# SELECT
- set alias with or without using `as`
```sql
...
return distinct rows, taking all columns into account
...
### Create a new `dvdrental` database
```sql
psql -U my_postgres -d postgres -c "CREATE DATABASE dvdrental OWNER my_postgres;"
```

### Restore the database `dvdrental` from a backup file
```sql
...
### Some initial command executions
```sql
-- check or list all databases
\l

-- switch to the `dvdrental` database
\c dvdrental

-- list all tables in the `dvdrental` database
\dt
```
...
```sql
SELECT rental_date, return_date, extract(DAY FROM return_date - rental_date) rental_period_as_days 
FROM rental 
WHERE NOT rental_period_as_days IS NULL
ORDER BY rental_period_as_days DESC;
```


### Calculating between different rows
```sql
...
How to fin the percentage difference in payment amounts from March to April 2007?
...
Method 3, using CTE and CASE WHEN together
```sql
with monthly_payments as (
  select
    sum(case when date_trunc('Month', payment_date) = '2007-03-01' then amount else 0 end) as march_payment,
